...
<?php if (!empty($page)) {
             ...
             $rows =mysqli_query($con,"SELECT id,pslug FROM product where slug='$page'  ORDER BY ordr" ) or die(mysqli_error($con));
             while($row=mysqli_fetch_array($rows)){
               $confirm = $row['id']; 
               $pslug = $row['pslug']; 
  ?>
        ...
        <div class="col-lg-7">
          <div class="widget-container fluid-height clearfix">
            <div class="heading" style="text-transform: capitalize;">
              ...
              <a href="products-<?php echo $pslug ?>"> <i class="fa fa-reply"></i>Back to products </a>
            </span>
            </div>
            <div class="widget-content padded clearfix">
               <form method="post" action="" enctype="multipart/form-data">
